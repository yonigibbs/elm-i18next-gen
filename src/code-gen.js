 ...
 * folders will go. Undefined/null means `parentFolder` is the root.
 */
const deleteRemovedItems = (parentFolder, generatedFiles, pathRelativeToRoot) => {
    fs.readdirSync(parentFolder).forEach(child => {
        ...
        if (fs.lstatSync(fullChildPath).isDirectory()) {
            ...
            const generatedFilePrefix = pathRelativeToRoot
                ? pathRelativeToRoot + path.sep + child + path.sep
                : child + path.sep
            ...
            const childFilePathRelativeToRoot = pathRelativeToRoot
                ? pathRelativeToRoot + path.sep + child
                : child
...
module.exports = (sourceFile, targetFolder, overwrite = false) => {
    ...
    if (overwrite)
        ...
        deleteRemovedItems(resolvedTargetFolder, files)
